...
컬러 테마 설정 화면의 위에서부터 순서대로, ‘라이트: 단계 1이 시작됨’, ‘다크: 단계 2가 시작됨’이라는 테마를 설정했다고 가정합니다.

단계 2 시작 시점에 라이트에서 다크로 바뀌기를 바라지만, 이 설정으로는 바뀌지 않습니다.

단계 1 → 단계 2로 진행된 경우, ‘단계 1이 시작됨’이라는 조건은 단계 2가 시작되어도 여전히 충족되기 때문입니다. 조건은 더 위에 있는 이미지가 우선되므로, 라이트의 조건이 충족되는 한 계속 다크로 바뀌지 않습니다.

개선 방법\* 라이트의 조건을 「단계1이 진행 중」으로 바꾼다

* 라이트의 조건을 「단계1이 시작했으며 단계2에  도달하지 않았다」로 바꾼다
